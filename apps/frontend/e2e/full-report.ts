...
interface PWTest {
  status?: string; // 'expected' | 'skipped' | 'unexpected' | 'flaky'
  results?: PWTestRun[];
}
interface PWSpec {
  title?: string;
...
function classify(spec: PWSpec): 'passed' | 'failed' | 'skipped' {
...
function flatten(suites: PWSuite[] | undefined, parent = ''): PWSuite[] {
  if (!suites) return [];
  const out: PWSuite[] = [];
...
function parsePlaywright(jsonPath: string): TestSuite[] {
  if (!fs.existsSync(jsonPath)) return [];
  const data = JSON.parse(fs.readFileSync(jsonPath, 'utf-8')) as PWReport;
...
function generateReport(suites: TestSuite[]): string {
